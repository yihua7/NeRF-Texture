...
class FFMLP(nn.Module):
    ...
    def forward(self, inputs, force_grad=False):
        # inputs: [B, input_dim]
        # return: [B, outupt_dim]
        ...
        outputs = ffmlp_forward(inputs, self.weights, self.input_dim, self.padded_output_dim, self.hidden_dim, self.num_layers, self.activation, self.output_activation, (not self.training) and (not force_grad), inputs.requires_grad)
