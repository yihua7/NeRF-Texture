...
class RayTracer():
    def __init__(self, vertices, triangles):
        ...
        if triangles.shape[0] <= 8:
            v_inf = 1e3 * np.ones([3, 3])
            f_inf = np.broadcast_to(np.arange(3)[None], [8, 3]) + vertices.shape[0]
            print('Expand v, f from ', vertices.shape, ' ', triangles.shape, end='')
            vertices = np.concatenate([vertices, v_inf], axis=0)
            triangles = np.concatenate([triangles, f_inf], axis=0)
            print(' to ', vertices.shape, ' ', triangles.shape)
    ...
    def trace(self, rays_o, rays_d, inplace=False):
        ...
        face_idx = - torch.ones([rays_o.shape[0]], dtype=torch.int64).contiguous()

        if not rays_o.is_cuda: rays_o = rays_o.cuda()
        if not rays_d.is_cuda: rays_d = rays_d.cuda()
        if not face_idx.is_cuda: face_idx = face_idx.cuda()
        ...
        self.impl.trace(rays_o, rays_d, positions, face_normals, depth, face_idx) # [N, 3]

        positions = positions.view(*prefix, 3)
        face_normals = face_normals.view(*prefix, 3)
        depth = depth.view(*prefix)

        return positions, face_normals, depth, face_idx