...
class NeRFRenderer(nn.Module):
    ...
    def sample(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, **kwargs):
        prefix = rays_o.shape[:-1]
    ...
    def run_cuda(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, **kwargs):
        ...
        if self.training:
            ...
            #     max_dist_rays = 1024
            #     max_dist_points = 512
            #     if max_dist_rays <= rays_o.shape[0]:
            #         rays_id_dist = np.random.choice(np.arange(rays_o.shape[0]), size=[max_dist_rays], replace=False)
            #     else:
            #         rays_id_dist = np.arange(rays_o.shape[0])
            #     rays_o_dist, rays_d_dist = rays_o[rays_id_dist].detach(), rays_d[rays_id_dist].detach()
            #     nears_dist, fars_dist = nears[rays_id_dist].detach(), fars[rays_id_dist].detach()
            #     z_vals = torch.linspace(0.0, 1.0, max_dist_points, device=device).unsqueeze(0) # [1, T]
            #     z_vals = z_vals.expand((max_dist_rays, max_dist_points)) # [N, T]
            #     z_vals = nears_dist.unsqueeze(-1) + (fars_dist.unsqueeze(-1) - nears_dist.unsqueeze(-1)) * z_vals # [N, T], in [nears, fars]
            #     sample_dist = (fars_dist.unsqueeze(-1) - nears_dist.unsqueeze(-1)) / max_dist_points
            #     z_vals = z_vals + (torch.rand(z_vals.shape, device=device) - 0.5) * sample_dist
            #     xyzs_dist = rays_o_dist.unsqueeze(-2) + rays_d_dist.unsqueeze(-2) * z_vals.unsqueeze(-1) # [N, 1, 3] * [N, T, 1] -> [N, T, 3]
            #     xyzs_dist = torch.min(torch.max(xyzs_dist, self.aabb_train[:3]), self.aabb_train[3:]) # a manual clip.
            #     sigmas_dist = self.density(xyzs_dist.reshape([-1, 3]))['sigma'].reshape(xyzs_dist.shape[:2])
            #     deltas_dist = z_vals[..., 1:] - z_vals[..., :-1] # [N, T-1]
            #     deltas_dist = torch.cat([deltas_dist, sample_dist * torch.ones_like(deltas_dist[..., :1])], dim=-1)
            #     alphas = 1 - torch.exp(-deltas_dist * self.density_scale * sigmas_dist.squeeze(-1)) # [N, T]
            #     alphas_shifted = torch.cat([torch.ones_like(alphas[..., :1]), 1 - alphas + 1e-15], dim=-1) # [N, T+1]
            #     weights = alphas * torch.cumprod(alphas_shifted, dim=-1)[..., :-1] # [N, T]
            #     s_vals = (1 / (z_vals + 1e-5) - 1 / (nears_dist.unsqueeze(-1) + 1e-5)) / (1 / (fars_dist.unsqueeze(-1) + 1e-5) - 1 / (nears_dist.unsqueeze(-1) + 1e-5) + 1e-5)
            #     s_deltas_dist = s_vals[..., 1:] - s_vals[..., :-1]
            #     s_deltas_dist = torch.cat([s_deltas_dist, (1 / max_dist_points) * torch.ones_like(s_deltas_dist[..., :1])], dim=-1)
            #     distortion_loss = eff_distloss(weights, s_vals, s_deltas_dist)
            ...
            if len(sigmas.shape) == 2:
                ...
                for key in data_dict.keys():
                    if data_dict[key] is not None and 'normal' in key:
                        weights_sum_data, _, data_composed = raymarching.composite_rays_train(sigmas.detach(), data_dict[key], deltas, rays)
                        data_composed = data_composed + (1 - weights_sum_data).unsqueeze(-1) * torch.zeros_like(bg_color)
            ...
            while step < max_steps:
                ...
                # if 'normal_error' in data_dict.keys() and data_dict['normal_error'] is not None:
                #     raymarching.composite_rays(n_alive, n_step, rays_alive[i % 2].clone(), rays_t[i % 2].clone(), sigmas, data_dict['normal_error'], deltas, weights_sum.clone(), depth.clone(), normal_error)
